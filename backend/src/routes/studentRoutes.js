const express    = require("express");
const router     = express.Router();
const jwt        = require("jsonwebtoken");
const User       = require("../models/Users");
const ctrl       = require("../controllers/studentController");

const protect = async (req, res, next) => {
  try {
    const token = req.headers.authorization?.split(" ")[1];
    if (!token) return res.status(401).json({ success: false, message: "Not authorized" });
    const decoded = jwt.verify(token, process.env.JWT_SECRET);
    req.user = await User.findById(decoded.id).select("-password");
    next();
  } catch (err) { res.status(401).json({ success: false, message: "Token invalid" }); }
};

router.get   ("/profile",           protect, ctrl.getProfile);
router.put   ("/profile",           protect, ctrl.updateProfile);
router.post  ("/appointments",      protect, ctrl.bookAppointment);
router.get   ("/appointments",      protect, ctrl.getAppointments);
router.delete("/appointments/:id",  protect, ctrl.cancelAppointment);
router.get   ("/documents",         protect, ctrl.getDocuments);
router.get   ("/applications",      protect, ctrl.getApplications);

module.exports = router;